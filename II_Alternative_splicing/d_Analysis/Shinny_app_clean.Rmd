...
```{r}
load('/lustre/scratch117/cellgen/team218/gp7/Joe/Jupyter/shiny_app_light.rda')
```
...
vert.names[ ,  manual_clusters:=final_clusters_vector[cluster] ]
vert.names[manual_clusters==37, `:=`(cell_type="Parietal Endoderm", Class="Extra embryonic")]



vert.names.final <- vert.names[ manual_clusters %in% c(1, 24, 8, 16, 35,18, 33, 28, 32, 12,30, 21, 34, 20, 37, 27)]
...
cluster_vector <- vert.names$manual_clusters
names(cluster_vector) <- vert.names$cluster
...
```{r}
pseudopool_matrix.zscore.PAGA.node <- pseudopool_matrix.zscore.PAGA[Coord=="15:76074607-76074657" ]
```
...
```{r}
accepted_gene_nodes <- pseudopool_matrix.zscore.PAGA[  , .(cov=sum(Total_Reads, na.rm=T)) , by=c("Coord", "Gene")][ ,  .(rank=frank(-cov ), Gene, cov) , by=c("Coord")][ rank==1, .(Coord, Gene)]


pseudopool_matrix.zscore.PAGA <- merge(pseudopool_matrix.zscore.PAGA, accepted_gene_nodes, by=c("Coord", "Gene"))

pseudopool_matrix.zscore.PAGA[ paste(Coord, Gene)  %in% accepted_gene_nodes, ]


pseudopool_matrix.zscore.PAGA[ , cluster:=as.character(cluster_vector[cluster] ) ]
...
```{r}
pseudopool_matrix.zscore.PAGA[Coord=="15:76074607-76074657" & pool_ID=="7_2"]
gene_count
PAGA
splicing_node_table
all_metadata
...
gene <- unique(pseudopool_matrix.zscore.PAGA.node$Gene)
...
node_plot <- ggplot() +
    ...
    point.padding = 0.3,
                label=vert.names$manual_clusters )  +
...
pseudopool_matrix.zscore.PAGA.node <- merge(pseudopool_matrix.zscore.PAGA.node, splicing_node_table[Gene %in% gene, ], by="Coord")
...
gene_sp <- merge(gene_count[Gene==gene[1],], all_metadata[, .(cell=sample, pseudo_bulk_ID)], by="cell")
...
This is the variable pannel alone


```{r, fig.height=9, fig.width=10}

node = "15:76074607-76074657"
...
```
	...
	15:77091812-77091854
	15:77094451-77094490
	
		2:153683608-153683677

```{r, fig.height=9, fig.width=10}

node = "2:153683608-153683677"
...
Dnmt3b−30
```{r}
splicing_node_table[mgi_symbol=="Rbfox2", ]
```


The following chunk has the full display


Here are some interesting nodes to look at
